...
export default function LoginButton() {
  ...
  if (!user) {
    return (
      ...
      >
        ...
        <span className="hidden sm:inline">{loading ? 'Bezig...' : 'Inloggen'}</span>
      </Button>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="hidden md:flex items-center gap-3 px-3 py-1.5 bg-neutral-50 rounded-lg border border-neutral-200">
        {user.photoURL && (
          <img
            src={user.photoURL}
        ...
        <div className="text-sm">
          ...
          {isAuthorized ? (
            <Badge variant="success" size="sm">✓ Geautoriseerd</Badge>
          ) : (
            <Badge variant="warning" size="sm">⚠ Niet geautoriseerd</Badge>
          )}
        </div>
      </div>
      <Button
